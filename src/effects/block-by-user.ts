...
export const blockCreditsByUserEffect: Firebot.EffectType<effectParams> = {
    definition: {
        id: "magecredits:blockCreditsByUser",
        name: "Credit Generator: Block Credits By User",
        description: "Blocks all credits attributed to a particular user without needing to restart Firebot.",
    ...
    onTriggerEvent: async (event) => {
        const { effect } = event;
        currentStreamCredits.blockCreditsByUser(effect.username);
        logger.info(`Blocked credits for user: ${effect.username}`);
    }
};
