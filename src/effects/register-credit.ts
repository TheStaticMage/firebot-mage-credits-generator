...
import type {
    Trigger,
    TriggersObject
} from "@crowbartools/firebot-custom-scripts-types/types/triggers";
import { currentStreamCredits } from "../credits-store";
import { firebot, logger } from "../main";
import { type CreditedUser, CreditTypes } from "../types";
...
export const registerCreditEffect: Firebot.EffectType<registerCreditEffectParams> =
    {
        definition: {
            ...
            description:
				"Register an event with the credits tracking system. This can register events such as cheers, donations, follows, gifts, raids, and subs based on the event metadata so that you do not need to specify parameters manually.",
...
class RegisterCreditsEffectController {
    async onTriggerEvent(event: { trigger: Trigger }) {
        const trigger = event.trigger;

        const eventSource = trigger.metadata?.eventSource?.id;
        if (!eventSource) {
            logger.error(
                `registerCreditEffect: Invalid event source provided for trigger. metadata: ${JSON.stringify(trigger.metadata)}`
            );
            return;
        }
        ...
        if (!eventType) {
            logger.error(
                `registerCreditEffect: Invalid event type provided for trigger. metadata: ${JSON.stringify(trigger.metadata)}`
            );
            return;
        }
        ...
        switch (eventSourceAndType) {
            ...
            case "twitch:cheer": {
                ...
                if (!username) {
                    logger.error(
                        `registerCreditEffect: No username provided for trigger. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                currentStreamCredits.registerCredit(CreditTypes.CHEER, {
                    ...
                    profilePicUrl:
						(trigger.metadata?.eventData?.profilePicUrl as string) || ""
                });
                logger.debug(
                    `Registered cheer from ${eventSourceAndType} for user ${username} (${forceNumber(trigger.metadata?.eventData?.bits)}).`
                );
                break;
            }
            ...
            case "twitch:bits-powerup-gigantified-emote": {
                ...
                if (!username) {
                    logger.error(
                        `registerCreditEffect: No username provided for bits powerup event. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                const bits = trigger.metadata?.eventData?.bits;
                if (bits == null || Number.isNaN(Number(bits))) {
                    logger.error(
                        `registerCreditEffect: No bits provided in the event data for bits powerup event. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                if (Number(bits) <= 0) {
                    logger.warn(
                        `registerCreditEffect: Bits provided for bits powerup event is zero or negative. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                logger.debug(
                    `Registered bits powerup event from ${eventSourceAndType} for user ${username} (${forceNumber(bits)}).`
                );
                break;
            }
            case "streamelements:donation": {
                ...
                if (!donor) {
                    logger.error(
                        `registerCreditEffect: No donor ('from') provided in the event data. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }

                const donationAmount = trigger.metadata?.eventData?.donationAmount;
                if (donationAmount == null || Number.isNaN(Number(donationAmount))) {
                    logger.error(
                        `registerCreditEffect: No donationAmount provided in the event data. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                const user = await viewerDatabase.getViewerByUsername(
                ...
                if (user) {
                    ...
                    logger.warn(
                        `registerCreditEffect: User '${donor}' not found in Firebot user database. Registering donation event as best guess.`
                    );
                }
                ...
                logger.debug(
                    `Registered donation/tip from ${eventSourceAndType} for donor ${donor} (${forceNumber(donationAmount)}).`
                );
                break;
            }
            case "twitch:charity-donation": {
                ...
                if (!username) {
                    logger.error(
                        `registerCreditEffect: No username provided for charity donation. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                if (donationAmount == null || Number.isNaN(Number(donationAmount))) {
                    logger.error(
                        `registerCreditEffect: No donationAmount provided in the event data for charity donation. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }

                const userDisplayName =
                    trigger.metadata?.eventData?.userDisplayName || username;
                ...
                logger.debug(
                    `Registered charity donation from ${eventSourceAndType} for user ${username} (${forceNumber(donationAmount)}).`
                );
                break;
            }
            ...
            case "twitch:follow": {
                const userName =
                    trigger.metadata.username || trigger.metadata.eventData?.username;
                const userDisplayName =
                    trigger.metadata.eventData?.userDisplayName || userName;
                const profilePicUrl = trigger.metadata.eventData?.profilePicUrl || "";
                if (!userName) {
                    logger.error(
                        `registerCreditEffect: No username provided for follow event. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                logger.debug(
                    `Registered follow from ${eventSourceAndType} for user ${userName as string}.`
                );
                break;
            }
            ...
            case "twitch:subs-gifted": {
                ...
                if (isAnonymous) {
                    logger.debug(
                        `registerCreditEffect: Anonymous subs gifted event detected. No username to register.`
                    );
                    return;
                }
                ...
                if (!username) {
                    logger.error(
                        `registerCreditEffect: No gifter username provided for subs gifted event. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                logger.debug(
                    `Registered subs gifted from ${eventSourceAndType} for user ${username} (${forceNumber(trigger.metadata?.eventData?.subCount || 1)}).`
                );
                break;
            }
            ...
            case "twitch:raid": {
                ...
                logger.debug(
                    `Registered raid from ${eventSourceAndType} for user ${username} (${forceNumber(trigger.metadata?.eventData?.viewerCount)}).`
                );
                break;
            }
            ...
            case "twitch:gift-sub-upgraded": {
                ...
                const amount = Math.max(
                    forceNumber(trigger.metadata?.eventData?.totalMonths),
                    1
                );
                ...
                logger.debug(
                    `Registered subscription from ${eventSourceAndType} for user ${username} for ${amount} months.`
                );
                break;
            }
            ...
            case "twitch:viewer-arrived": {
                ...
                if (
                    !metadata ||
					!metadata.eventData ||
					!metadata.eventData.chatMessage
                ) {
                    logger.error(
                        `registerCreditEffect: Missing metadata for viewer-arrived event.`
                    );
                    return;
                }
                ...
                const chatMessage = metadata.eventData
                    .chatMessage as partOfFirebotChatMessage;
                if (!chatMessage || !chatMessage.username) {
                    logger.error(
                        `registerCreditEffect: Invalid chatMessage provided for chat-message/viewer-arrived event. metadata: ${JSON.stringify(trigger.metadata)}`
                    );
                    return;
                }
                ...
                if (eventSource === "mage-kick-integration") {
                    userEntry.username = chatMessage.username.endsWith("@kick")
                        ? chatMessage.username
                        : `${chatMessage.username}@kick`;
                    userEntry.userDisplayName =
                        chatMessage.userDisplayName ||
						userEntry.username.substring(0, userEntry.username.length - 5);
                }

                if (eventSource === "mage-youtube-integration") {
                    userEntry.username = chatMessage.username.endsWith("@youtube")
                        ? chatMessage.username
                        : `${chatMessage.username}@youtube`;
                    userEntry.userDisplayName =
                        chatMessage.userDisplayName ||
						userEntry.username.substring(0, userEntry.username.length - 8);
                }
                ...
                if (chatMessage.roles.includes("mod") || chatMessage.isMod) {
                    ...
                    logger.debug(
                        `Registered moderator chat for user ${userEntry.username}.`
                    );
                }
            ...
            default:
                logger.error(
                    `registerCreditEffect: Unknown event type "${eventType}" provided for trigger. metadata: ${JSON.stringify(trigger.metadata)}`
                );
                return;
        }
    }
...
export const registerCreditsEffectController =
    new RegisterCreditsEffectController();
